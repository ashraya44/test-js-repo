...
RUN iptables -t nat -A OUTPUT -j pse-proxy 
RUN PSE_IP=$(getent hosts pse-proxy | awk '{ print $1 }') 
RUN echo "PSE_IP is ${PSE_IP}" 
RUN iptables -t nat -A "$PSE_CONTAINER_NAME" -p tcp -m tcp --dport 443 -j DNAT --to-destination "${PSE_IP}:12345" 
